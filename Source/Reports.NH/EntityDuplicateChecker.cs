...
{
    public class EntityDuplicateChecker : Reports.Data.Repositories.IEntityDuplicateChecker
    {
        ...
        /// </summary>
        public bool DoesDuplicateExistWithTypedIdOf<TId>(Reports.Core.Domain.Entities.IEntityWithTypedId<TId> entity)
        {
            ...
            ICriteria criteria, PropertyInfo signatureProperty, object propertyValue)
        {
            criteria.Add(
                propertyValue != null
                    ? Restrictions.Eq(signatureProperty.Name + ".Id", ((Reports.Core.Domain.Entities.IEntityWithTypedId<TId>)propertyValue).Id)
        ...
        private void AppendSignaturePropertyCriteriaTo<TId>(ICriteria criteria, Reports.Core.Domain.Entities.IEntityWithTypedId<TId> entity)
        {
            foreach (var signatureProperty in entity.GetSignatureProperties())
            {
                ...
                if (propertyType.IsEnum)
                {
                    ...
                    propertyType.GetInterfaces().Any(
                        x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(Reports.Core.Domain.Entities.IEntityWithTypedId<>)))
                {
