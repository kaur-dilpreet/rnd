$.fn.treeView = function (option) {
        ...
        if (!$(this).parents('.treeView').hasClass('disabled')) {
            if (!$(this).hasClass('selected')) {
                ...
                if (option != undefined && option['onSelect'] != undefined)
                    option.onSelect($(this).parents('li').data('uniqueid'), $(this).parents('li').data('userid'));
            }
        }
    });
    ...
    $.fn.treeNodeClick = function (e) {
        ...
        if (!$(this).parents('.treeView').prop('loading')) {
            ...
            var url = $(this).parents('ul.treeView').data('geturl');

            thisLi.addClass('loading');
            if (userId == '' && uniqueId == '') {
                url = $(this).parents('ul.treeView').data('geturl').replace('getfoldersforuser', 'getallfoldersforuser');
                $.ajax({
                    cache: false,
                    url: url,                    
